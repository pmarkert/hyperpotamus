...
Plugins.prototype.findPluginForAction = function findPlugin(action) {
	...
	if (matching_plugin_names.length > 1) {
		throw { message: "Multiple plugins available to process action - " + JSON.stringify(action) + ", " + JSON.stringify(plugins), action: action, matching_plugin_names: matching_plugin_names };
	}
