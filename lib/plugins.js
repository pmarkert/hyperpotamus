...
var reservedProperties = ["on_success", "on_failure", "debugger", "plugin"];
...
Plugins.prototype.findPluginForAction = function findPlugin(action) {
	...
	if (matching_plugin_names.length == 0) {
		throw new verror.VError({ name: "NoMatchingPluginsError", info: { candidate_properties, action }}, `No Plugins found to process action [${action.path}].`);
	}
	if (matching_plugin_names.length > 1) {
		throw new verror.VError({ name: "MultipleMatchingPluginsError", info: { candidate_properties, action, matching_plugin_names }}, `Multiple plugins found to process action [${action.path}].`);
	}
...
function loadPlugins(pathToSearch, basePath) {
	...
	if (stat.isDirectory()) {
		var result = _.chain(fs.readdirSync(pluginPath))
			.filter(isJavascript)
			.map(function(p) { return path.resolve(pluginPath, path.basename(p, ".js")); })
			.map(loadPlugin)
			.filter(isPlugin)
			.map(initPlugin)
			.keyBy("name")
	...
	// Is it a file?
		...
		throw new Error("Cannot load plugin from non-javascript file: " + pluginPath);
	}
}
