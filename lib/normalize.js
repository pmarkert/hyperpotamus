...
module.exports = function (script, plugins) {
	...
	var normalizing_plugins = _.filter(plugins, function (plugin) {
		...
		if (script.steps) { // Check to see if it is the wrapper (having .steps)
			...
			if (script.version) {
				...
				if (!semver.satisfies(version, script.version)) {
					var message = "Script requires hyperpotamus version: " + script.version + ", current version " + version + " does not meet this requirement.";
	...
	function normalize_action(action, step, normalized) {
		...
		if (action.on_success) {
			...
			action.on_success = normalize_actions(action.on_success, step, normalized);
		}
		return action;
	}
}
