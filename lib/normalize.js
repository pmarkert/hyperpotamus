...
module.exports = function(step) {
	...
	for(var i=0;i<step.validation.length;i++) {
		if(!_.isObject(step.validation[i])) { // Shortcut syntax
			...
			if(_.isObject(step.validation[i].headers)) {
				for(var key in step.validation[i].headers) {
					var re_match = extract_regex(step.validation[i].headers[key]);
					step.validation[i].headers[key] = re_match ? { regex : re_match } : { text : step.validation[i].headers[key] }
				}
			}
