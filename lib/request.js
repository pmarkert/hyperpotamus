...
module.exports = function(request, context, callback) {
	...
	if(request.socks) {
		...
		request.agent = new require("socks5-http-client/lib/Agent")(request.socks);
	}
	else if(context.defaults.socks) {
		logger.info("Setting up socks proxy for request.");
		request.agent = new require("socks5-http-client/lib/Agent")(context.defaults.socks);
	}
