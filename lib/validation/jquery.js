...
module.exports.process = function(context) {
        ...
        jsdom.env(context.body, function(err, window) {
                if(err) {
                        return context.callback("Error parsing document - " + err, context.body, null);
                }
                var $ = jquery(window);
                ...
                if(context.validation.count) {
                        ...
                        if(matches.length!==compare) {
                                return context.callback("Expected count of matches did not match", compare, matches.length);
                        }
                }
                if(context.validation.capture) {
                        async.eachSeries(matches, function(element, cb) {
                                ...
                                return context.callback(err, compare, value);
                        });
                }
        });
}
