...
function validate_regexp(context) {
	var err;
	if(!regex_capturevalidate(context.validation.regex, context.body, context.session)) {
		err = "Body did not match regex";
	};
	return context.callback(err, context.validation.regex, context.body);
}

function validate_true_regexp(context) {
	var err;
	...
	return context.callback(err, re, context.body);
}
...
module.exports.process = function(context) {
	if(_.isRegExp(context.validation)) return validate_true_regexp(context);
	else if(context.validation.regex) return validate_regexp(context);
}
