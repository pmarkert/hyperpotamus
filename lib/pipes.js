...
exports.random = function (val, min, max, whole) {
	if(!_.isNil(min)) { 
		...
		return _.random(min, max, asFloating);
	}
...
exports.start_of = function start_of(val, period) {
	return coerce_date(val).clone().startOf((period || "day").trim());
};

exports.end_of = function end_of(val, period) {
...
exports.match = function (val, pattern, group) {
	var result = extract_regex(pattern).exec(val);
	if (!group) {
		return result;
	}
	else {
...
exports.replace = function replace(val, pattern, replacement) {
	return val.replace(extract_regex(pattern), replacement);
};

exports.trim = function trim(val) {
	return val.toString().trim();
};

function extract_regex(pattern) {
