...
exports.utc_now = function() {
	return moment.utc();
}

exports.date = function (val) {
	return coerce_date(val).startOf("day");
};
...
function coerce_date(val) {
	if (!val._isAMomentObject && !(val instanceof Date)) {
		if(_.isString(val)) {
			val = val.trim();
		}
		val = new Date(val);
...
exports.date_add = function (val, count, interval) {
	return moment(coerce_date(val)).add(parseInt(count), interval.trim());
};
...
exports.resolve_url = function (relative, base) {
	return url.resolve(base.trim(), relative.trim());
};

exports.match = function(val, pattern, options) {
	return new RegExp(_.defaultTo(pattern,".+").trim(), _.defaultTo(options,"").trim()).exec(val);
};
