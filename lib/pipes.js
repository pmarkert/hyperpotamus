...
exports.date_format = function (val, format, timezone) {
	...
	return val.format(trim(format));
};

exports.date_parse = function (val, format, timezone) {
	return get_parser(timezone)(val, trim(format));
};
...
exports.today = function (val) {
	return exports.day(exports.now(val));
};


// Formats a date according to the specified moment.js format
exports.utc_format = _.partialRight(exports.date_format, "UTC");
exports.format_utc = exports.utc_format;

exports.utc_parse = _.partialRight(exports.date_parse, "UTC");
exports.parse_utc = exports.utc_parse;

// Adds/subtracts to/from a date
exports.date_add = function (val, count, interval) {
	return coerce_date(val).add(parseInt(count), trim(interval));
};
...
exports.hash = function (val, algorithm) {
	var hash = crypto.createHash(trim(algorithm));
	hash.update(val);
...
exports.resolve_url = function (relative, base) {
	return url.resolve(trim(base), trim(relative));
};

exports.match = function(val, pattern, options, group) {
	var result = new RegExp(trim(pattern,".+"), trim(options,"")).exec(val);
	if(!group) {
		return result;
	}
	else {
		return result[parseInt(trim(group))];
	}
};
