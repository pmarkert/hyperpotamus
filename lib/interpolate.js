		...
		if(date_format) {
			if(parameters) {
				...
				if(_.isUndefined(object)) {
					throw { message: "Target of date-format must be a moment/date object", expression: match, key : key, data : data };
				}
				if(!object._isAMomentObject) {
					...
					if(!object.isValid()) {
						throw { message: "Could not convert to date", expression: match, key : key, data : data };
					}
				}
			...
			if(_.has(data, key)) {
				...
				if(optional)
					...
					throw { message : "No value found for non-optional replacement '" + key, expression: string, data: data };
			}
		}
		if(delimiter_quote) {
