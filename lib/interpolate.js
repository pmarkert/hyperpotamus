...
var MissingKeyError = require("./error/MissingKeyError");
...
var options = {
	...
	undefinedResult: function undefinedResult(property, tag, context, child, filters, markup) {
		...
		throw new MissingKeyError(property, tag);
	},

	pipeNotFound: function pipelineError(pipe, err) {
		logger.error(`Error in pipe '${pipe}' during interpolation - ${err}`);
		throw new Error(`Executing pipe [${pipe}]\n  caused by: ${err}`);
	}
};
