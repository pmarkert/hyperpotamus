...
var options = {
	...
	undefinedResult: function undefinedResult(property, tag, context, child, filters, markup) {
		...
		throw new verror({ name : "MissingKeyError", constructorOpt: module.exports, info: { markup: { property, tag, context, child }}}, `No matching key was found in the session\n Key: ${property}\n Tag: ${tag}`);
	},
	
	pipeNotFound: function pipeNotFound(pipe) {
			throw new verror({ name: "PipeNotFoundError", info: { pipe }}, `Pipe function: '${pipe}' could not be found`);
	},

	pipeExecutionError: function pipelineError(pipe, err) {
		throw new verror({ name : "PipeExecutionError", info: { pipe }, cause: err }, `Error in pipe '${pipe}' during execution`);
	}
};
