...
function Context(session, processor, _processAction, _handleDirective, findStep) {
	...
	this.clone = function clone(with_session_data) {
		// TODO - Attempted to replace this with a simpler _.clone, but then shared_properties were failing from foreach tests
		var new_session = _.defaults(with_session_data, _.cloneDeep(self.session));
		var new_context = new Context(new_session, self._processor, self._raw_process_action, self._raw_handle_directive, self.findStep);
		new_context._cookieStore = _.cloneDeep(self._cookieStore);
		new_context._requestDefaultsStore = _.cloneDeep(self._requestDefaultsStore);
	...
	this.setSessionValue = function setSessionValue(path, value) {
		...
		for (var i = 0; i < arrays.length; i++) { // Increment the index for each array in the list
			if (!_.isArray(_.get(self.session,arrays[i]))) { // If it's not an array, pretend it was an array of one
				...
				if (array.currentIndex < array.length - 1) { // If we still have elements
					...
					logger.trace("Iteration for " + arrays[i] + " exhausted.");
	...
	this.requestDefaultsStore = function requestDefaultsStore() {
