...
function Context(session, processor, process_action, handle_directive) {
	...
	this.raw_process_action = process_action;
	this.raw_handle_directive = handle_directive;
	...
	this.clone = function(shared_properties) {
		shared_properties = _.isNil(shared_properties) ? [] : _.castArray(shared_properties);
		// Deep clone except for the shared_properties
		var new_session =_.cloneDeepWith(this.session, function(value, key, object, stack) {
			if(_.includes(shared_properties, key)) {
				return object[key];
			}
		});
		return new Context(new_session, this.processor, this.raw_process_action, this.raw_handle_directive);
	};
