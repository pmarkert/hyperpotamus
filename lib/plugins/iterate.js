	...
	if(this.debugger) debugger;
        ...
        for(var i=0; i<this.iterate.length; i++) { // Increment the .index for each array in the array of [iterate]
                if(!_.isArray(context.session[this.iterate[i]])) { // If it's not an array, pretend it's an array of one (and jump right away)
                        ...
                        if(!context.session[this.iterate[i] + ".index"] ) { // This is the first time, so the index was 0 before incrementing
                                context.session[this.iterate[i] + ".index"] = 0;
                        }
                        if(context.session[this.iterate[i] + ".index"] < context.session[this.iterate[i]].length - 1) { // If we still have elements
                                context.session[this.iterate[i] + ".index"]++;
                        }
                        else { // We reached the end
                                delete(context.session[this.iterate[i]+".index"]);
                                reached_end = true;
                        }
                }
        }
