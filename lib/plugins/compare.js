...
module.exports.process = function (context) {
	...
	if (!_.isArray(this.compare.array)) {
		throw new verror.VError({ name: "InvalidComparisonType" }, "Comparison target must be an array");
	}
	for (var i = 0; i < this.compare.array.length; i++) {
		actual = this.compare.array[i];
		if(_.isObject(actual) ||  _.isArray(actual)) {
			throw new verror.VError({ name: "InvalidComparisonValue" }, "Comparison value at index %s must not be an array or object. Value: %s", i, JSON.stringify(actual));
		}
		if (expected == undefined) {
			...
			if (!compare_values(expected, actual, this.compare.operator)) {
				throw new verror.VError({ name: "ComparisonFailedError" }, "Comparison mis-match at position %s Expected: %s %s %s", i, expected, this.compare.operator, actual );
			}
			expected = actual;
		}
	}
