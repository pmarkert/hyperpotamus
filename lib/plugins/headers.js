	...
	if(this.debugger) debugger;
        ...
        for(var key in this.headers) {
                ...
                if(this.headers[key].text) {
                        compare = this.headers[key].text;
                        if(compare !== value) {
                                throw new Error("Header " + key + " did not match text value", compare, value);
                        }
                }
                if(this.headers[key].regex) {
                        compare = this.headers[key].regex;
                        if(!regex_helper.capture_validate(this.headers[key].regex, value, context.session)) {
                                throw new Error("Header " + key + " did not match regex value", compare, value);
                        }
                }
        }
}
