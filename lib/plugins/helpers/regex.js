...
function non_global(re, value_to_match, session) {
...
function global(re, value_to_match, session) {
	var first = true;
	var matched;
...
module.exports.capture_validate = function (regex, value_to_match, session) {
	...
	if (!re.global) {
		return non_global(re, value_to_match, session);
	}
	else {
		return global(re, value_to_match, session);
	}
};
