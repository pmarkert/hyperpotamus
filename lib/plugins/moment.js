...
module.exports.process = function(context) {
	...
	if(_.isObject(this.moment.utc_parse)) {
		for(var key in this.moment.utc_parse) {
			context.session[key] = parse(this.moment.utc_parse[key], true);
		}
	}
...
function _parse(toParse) {
	...
	if(_.isString(toParse)) {
		return moment(new Date(toParse));
	}
	...
	if(_.isObject(toParse)) {
		...
		if(_.isString(toParse.format)) {
			return moment(toParse.value, toParse.format);
		}
		else {
			return moment(toParse.value);
		}
	}
}
