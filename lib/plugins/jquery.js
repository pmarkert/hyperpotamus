...
module.exports.process = function (context) {
	...
	if (this.count) {
		...
		if (matches.length !== compare) {
			return { message: "Expected count of matches did not match", expected: compare, actual: matches.length };
		}
	}
	if (this.capture) {
		for (var key in this.capture) {
			...
			if (_.isArray(expression)) {
				...
				if (expression.length == 0) {
					return { message: "Capture value array must not be empty - key: " + key };
				}
				if (expression.length > 1) {
					return { message: "Capture value array must not have multiple elements - key: " + key };
				}
