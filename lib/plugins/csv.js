			...
			try {
				...
				if (_.has(this.csv.mapping, fieldNames[i])) {
					...
					if(!_.has(context.session, fieldNames[i])) {
						throw new error.MissingKeyError(fieldNames[i], "CSV fields");
					}
				...
				debugger;
				var to_throw = new Error("processing csv field - " + fieldNames[i] + ". Nested error is - " + err);
				to_throw.stack = err.stack;
				throw to_throw;
			}
		}
		context.options.emit(result, this.channel);
		exhausted = context.iterate_arrays(_.union(arrays, _.castArray(this.csv.iterate)));
