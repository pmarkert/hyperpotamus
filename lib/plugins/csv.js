...
module.exports.process = function (context) {
	...
	this.csv.fields = context.interpolate(this.csv.fields);
	this.csv.header = context.interpolate(this.csv.header);
	...
	while (!exhausted) {
		...
		for (var i = 0; i < fieldNames.length; i++) {
			...
			if (_.has(this.csv.mapping, fieldNames[i])) {
				value = context.interpolate(this.csv.mapping[fieldNames[i]]); // If a mapping exists, use it.
			}
			else {
				value = context.interpolate("<%!" + fieldNames[i] + "%>"); // retrieve the value (not as a string)
			}
			if (_.isArray(value)) {
				has_arrays = true;
