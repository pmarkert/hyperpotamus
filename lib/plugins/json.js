...
module.exports.process = function(context) {
	...
	if(_.isArray(this.json)) {
		for(var i=0;i<this.json.length;i++) {
			...
			if(!result) {
				return { message : "Could not find element in JSON", target : target, json_path: this.json[i] };
			}
		}
	}
	else {
		for(var key in this.json) {
			...
			if(result.length==0) {
				return { message : "Could not find element in JSON", target : target, json_path: this.json[key] };
			}
