...
module.exports.process = function(context) {
	...
	if(_.isArray(this.json)) {
		for(var i=0;i<this.json.length;i++) {
			var result = jsonpath.eval(target, this.json[i]);
			if(!result) {
				throw new Error("Could not find element in JSON", this, this.json[i]);
			}
		}
	}
	else {
		for(var key in this.json) {
			var result = jsonpath.eval(target, this.json[key]);
			if(result.length==0) {
				throw new Error("Could not find element in JSON", this, this.json[key]);
			}
