...
function evaluate(target, key, debug) {
	...
	try {
		...
		if (!isOptional && result.length === 0) {
			var err = new Error("Could not find non-optional element in JSON - " + key);
			err.code = 100;
			throw err;
		}
		return result;
		...
		for (var key in this.json) {
			...
			if (array_capture) {
				if (this.json[key].length != 1) {
					throw { message: "Array capture for JSON must have a single element", target: target, path: this.json[key] };
				}
