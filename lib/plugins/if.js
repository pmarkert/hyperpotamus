...
function process(context, callback) {
	...
	context.validate.process_action(_if, context, function(err) {
		if(err) {
			logger.trace("if evaluated to false - " + JSON.stringify(_if));
			if(!_.isUndefined(self["else"])) {
				var _else = context.interpolate(self.else, context.session);
				context.validate.process_actions(_else, context, callback);
			}
			...
			logger.trace("if evaluated to true - " + JSON.stringify(_if));
