...
var log_func = function (logger_name, level, message) {
	console.log(level[2]("[" + moment().format() + "] [" + logger_name + "] [" + level[1] + "] - ") + message);
}

module.exports.set_level = function (level) {
	if (_.isNumber(level)) {
		logging_level = _.find(levels, function (p_level) {
			return p_level[0] === level
		});
	}
	else if (_.isString(level)) {
		logging_level = _.find(levels, function (p_level) {
			return p_level[1] === level
		});
	}
	else {
		logging_level = _.includes(levels, level)>=0 ? level : null;
	}
	if (logging_level == null) {
		console.log("Invalid logging level set - " + level + " trace logging will be performed.");
		logging_level = levels.trace;
	}
}
