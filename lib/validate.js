	...
	function process_action(action, context, callback) {
		...
		async.eachSeries(interested_plugins, function(plugin, action_callback) {
			if(plugin.process.length==2) { // Async plugin
				plugin.process.call(action, context, function(err, compare, value) {
				...
				try {
					...
					return action_callback({ message: err + ": " + err.compare + " vs. " + err.value, jump_to_key: action.on_failure });
				}
			}
			else {
