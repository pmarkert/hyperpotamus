...
function is_valid(validation, session, response, body) {
	if(validation.text) 
		...
		return regex_capturevalidate(validation.regex, body, session);
	}
		...
		for(var key in validation.headers) {
			...
			var response_value = response.headers[key];
			if(validation.headers[key].text && validation.headers[key].text !== response_value)
				return false;
			if(validation.headers[key].regex && !regex_capturevalidate(validation.headers[key].regex, response_value, session))
				return false;
		}
		return true;
	}
