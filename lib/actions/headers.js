...
module.exports.process = function (context) {
	...
	if (_.isNil(response)) {
		throw new verror.VError({
			name: "InvalidActionPlacement.headers",
			info: {
				path: this.path + ".headers"
			}
		}, "The headers action is only valid for use within the .response of a request action.");
	}
	var actual, expected;
	for (var key in this.headers) {
		actual = response.headers[key.toLowerCase()]; // Node.js lowercases all headers so we have to match for case insensitivity
