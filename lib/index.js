...
function process_file(filename, session, done) {
	fs.readFile(filename, "utf-8", function(err, script_text) { 
		if(err) return done(err); 
		process_script(script_text, session, done); 
	});
}

function process_script(script_text, session, done) {
	...
	return process_step(_.first(normalized), normalized, session, done);
}

// TODO - Add process_url to read from a url

function process_step(step, script, session, done) {
