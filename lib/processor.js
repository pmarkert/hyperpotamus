...
function Processor(options) {
	...
	self.add_plugins = function add_plugins(plugins) {
	...
	function process_action(action, context, callback) {
		...
		var plugins = _.keys(_.pick(self.plugins, _.difference(_.keys(action), reserved_properties)));
		...
		var plugin = self.plugins[plugins[0]];
