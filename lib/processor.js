	...
	function process(script, session, firstStep) {
		...
		if (script.version) {
			...
			if (!semver.satisfies(hyperpotamus_version, script.version)) {
				return Promise.reject(new verror.VError({
					name: "ScriptVersionMismatchError",
					info: {
		...
		if (!step) {
			throw new verror.VError({
				name: "StartingStepNotFoundError",
				info: {
					goto: firstStep || "#1"
				}
			});
		...
		function processStep(step, context) {
			...
			return processAction(step.actions, context)
				.then(() => {
					...
					if (jumpResult !== null) {
						return handleDirective(verror.info(jumpResult), step);
					}
		...
		function processAction(action, context) {
			...
			return promiseActionExecution(action, context)
				.then(context => {
