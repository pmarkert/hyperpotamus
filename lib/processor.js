	...
	function process(script, session, first_step) {
		...
		if(script.version) {
			...
			if (!semver.satisfies(hyperpotamus_version, script.version)) {
				return Promise.reject(new verror.VError({ name : "VersionMismatchError", info: { current_version: hyperpotamus_version, required_version: script.version }}));
			}
		}
		...
		if (!step) {
			throw new verror.VError({ name : "StepNotFoundError", info: { goto: first_step || "#1" }});
		}
		...
		function process_step(step, context) {
			...
			return process_action(step.actions, context).then(() => {
				...
				if(verror.hasCauseWithName(err, "JumpResult")) return handle_directive(verror.info(err), step)
				// If it's not already an error object, let's wrap it in one
				if(!(err instanceof verror.VError)) {
					err = new verror.VError({ name: "StepProcessingError", cause: err });
				}
		...
		function handle_directive(directive, step) {
			...
			// Named target
				...
				if (!next_step) {
					...
					var err = new verror.VError({ name: "StepNotFoundError", info: { goto: directive.goto, step: step }});
					err.step = step;
		...
		function process_action(action, context) {
			...
			return promise_action_execution(action, context).then(context => {
				...
				if (action.on_failure) {
					...
					var throwable;
					if(_.has(err, "goto")) {
						throwable = new verror.VError({ name : "JumpResult", info: { goto: err.goto, context }});
					}
					else {
						throwable = new verror.VError({ name : "ActionProcessingError", cause: err, info: { action, context } });
					}
					throw throwable;
				}
			});
		...
		function promise_action_execution(action, context) {
			try {
				...
				if(context.options.safe && !plugin.safe) {
					throw new verror.VError({ name: "UnsafePluginError", info: { action: action, plugin: plugin.name }}, "Unsafe plugin cannot be executed in safe-mode");
				}
				...
				if (plugin.process.length == 2) { // Async plugin
					...
					throw new verror.VError({ name: "InvalidPluginError", info: { plugin: plugin.name } }, "Unexpected number of arguments for plugin.process() method. (Must be 1 or 2 arguments)");
				}
			}
			catch(err) {
				err.jse_info.action = action;
				throw err;
			}
		}
	}
