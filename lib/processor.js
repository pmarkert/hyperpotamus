...
function process_script(script, session, options, first_step) {
	if (!options) {
	...
	// Initialize default response validation
	session["response_defaults"] = [ { status : 200 } ];
	var context = {
		options: options,
		session: session,
...
function instance(options) {
	...
	return {
		...
		process: function (script, session, callback, first_step) {
			var local_options = options;
			if (_.isFunction(callback)) {
				local_options = { done: callback };
				_.defaults(local_options, options);
			}
			process_script(script, session, local_options, first_step);
		}
	};
};
