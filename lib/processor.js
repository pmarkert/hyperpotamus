...
function Processor(options) {
	...
	function process(script, session, first_step) {
		...
		if (!step) {
			throw new Error("Could not find starting_step - " + (first_step || "#1")); // TODO - return context too
		}
			...
			if (err.goto) {
				...
				if (err.goto === "SELF") { // Jump to the same request again (warning, could cause indefinite loops)
					...
					if (next_step) {
						...
						throw new Error("Could not find step", err); // , step: err.goto }, context); TODO
					}
				}
			}
			else {
				...
				throw new Error("Validation error", err); //, error: err }, context); // TODO
			}
		});
	}
