...
function Processor(options) {
	...
	function process(script, session, firstStep) {
		...
		var step = firstStep ? findStep(firstStep): _.first(script.steps);
		...
		function processStep(step, context) {
			...
			return processAction(step, context)
				.then(() => {
			...
			return promiseActionExecution(action, context)
				...
				.catch(err => !(err instanceof ProcessingDirective), err => {
					...
					if (action.on_failure) {
						...
						if (_.has(err, "goto")) {
							throw new ProcessingDirective({ goto: err.goto, context });
						}
						else {
							throw new verror.VError({
								...
								info: _.defaults(verror.info(err), {
									action,
									path: action.path,
									session: _.without(context.session, [ "hyperpotamus" ])
								})
