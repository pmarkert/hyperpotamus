	...
	function process(script, session, first_step) {
		...
		if(script.version) {
			...
			if (!semver.satisfies(hyperpotamus_version, script.version)) {
				return Promise.reject(new error.VersionMismatchError(hyperpotamus_version, script.version));
			}
		}
		...
		function handle_directive(directive, step) {
			...
			// Named target
				...
				if (!next_step) {
					...
					var err = new error.StepNotFoundError(directive.goto);
					err.step = step;
		...
		function process_action(action, context) {
			...
			logger.trace("Processing action - " + yaml.dump(action));
			return promise_action_execution(action, context).then(context => {
				...
				var throwable = err; 
				if(!_.has(err, "goto") && err.name!="ActionProcessingError") {
					throwable = new error.ActionProcessingError(err, action);
				}
				err.context = err.context || context;
				if (action.on_failure) {
					logger.info(`Handled error, now processing on_failure actions.\n Action: ${JSON.stringify(action,null,2)}\n Error: ${throwable}`);
					context.error = err;
