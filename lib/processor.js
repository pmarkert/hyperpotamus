...
function Processor(options) {
	...
	function process_step(step, script, context) {
		...
		return process_action(step.actions, context).then(() => {
			...
			if(_.has(e, "goto")) return handle_goto(e, step, script, context)
			e.step = step;
			throw e; 
		});
	}
	...
	function handle_goto(directive, step, script, context) {
		...
		// Named target
			...
			if (!next_step) {
				...
				var error = new error.StepNotFoundError(directive.goto); // , step: err.goto }, context); TODO
				error.step = step;
...
debugger;
	...
	function promise_action_execution(action, context) {
		...
		if (plugin.process.length == 2) { // Async plugin
			...
			throw new Error("Unexpected number of arguments for [${plugin.name}] .process() method. (Must be 1 or 2 arguments)");
		}
	}
}
