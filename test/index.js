	...
	return Promise.all(fs.readdirSync(path.join(__dirname, dir))
		...
		.map(filename => it(path.join(dir, filename), function() {
			return new hyperpotamus.Processor({ safe : false, auto_load_plugins: true })
				...
				.then(() => {
					if(should_expect_failure) throw new Error("Should have raised an error, but didn't");
				}).catch(err => {
					if(!should_expect_failure) throw err;
				});
			})
		)
	);
...
describe("HTTP Tests", function() {
	before(function(done) {
		var processor = new hyperpotamus.Processor({ safe : false, auto_load_plugins: true }); // Force load all plugins to prevent the first unit test from getting penalized time-wise
